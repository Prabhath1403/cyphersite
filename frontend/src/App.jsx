      ...
      >
        ...
        <div className="px-3.5 pt-3 pb-2">
          <button
            ...
            className="w-full flex items-center justify-between px-3 py-2 rounded-lg bg-white/[0.06] hover:bg-white/[0.1] border border-white/15 text-xs text-silver-200 transition-all group shadow-glow-silver/10 cursor-pointer"
            title="Open Project History (ChatGPT style)"
          >
            <div className="flex items-center gap-2 truncate">
              <History className="w-4 h-4 text-silver-300 group-hover:rotate-[-20deg] transition-transform" />
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-3.5 py-2.5 rounded-lg text-sm font-medium transition-all duration-150 ${
                  isActive
                    ? 'bg-white/[0.08] text-white border border-white/20 shadow-glow-silver'
                    : 'text-gray-400 hover:text-gray-200 hover:bg-white/[0.04]'
                }`}
              >
                <Icon className={`w-4 h-4 transition-colors ${isActive ? 'text-silver-200' : 'text-gray-400 group-hover:text-gray-200'}`} />
                <span className="flex-1">{label}</span>
                {isActive && <ChevronRight className="w-3.5 h-3.5 text-silver-300/60" />}
              </Link>
            );
          })}
        </nav>
  ...
  return (
    <header className="sticky top-0 z-30 h-16 bg-navy-950/80 backdrop-blur-xl border-b border-white/[0.08] px-6 flex items-center justify-between">
      <div className="flex items-center gap-3">
        ...
        <button
          onClick={openHistory}
          className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-navy-900/90 hover:bg-navy-800 border border-white/20 text-xs text-gray-200 transition-all shadow-glow-silver/15 cursor-pointer group"
          title="Open Project History (ChatGPT style)"
        >
          <History className="w-4 h-4 text-silver-300 group-hover:rotate-[-20deg] transition-transform" />
          <span className="text-gray-400 hidden sm:inline">Project:</span>
          <span className="text-silver-200 font-mono font-semibold max-w-[170px] truncate">
...
function AppContent() {
  ...
  return (
    <div className="flex min-h-screen bg-navy-950 text-gray-100 selection:bg-white/20 selection:text-white">
